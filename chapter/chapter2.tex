...
\begin{itemize}
    ...
    Các phép quay, tịnh tiến, co giãn, cắt, biến đổi tuyến tính \+ tịnh tiến (warp affine) và biến đổi phối cảnh (warp perspective). Đặc biệt quan trọng trong bài toán căn chỉnh ảnh (image alignment) giữa ảnh chụp thực tế và ảnh mẫu (golden template).
...
\begin{figure}[h]
    ...
    \caption{Các tác vụ được hỗ trợ bởi YOLOv8}
    \label{fig:yolov8}
\end{figure}

Ultralytics YOLOv8 là một nền tảng thị giác máy tính hiện đại, được thiết kế để hỗ trợ đa dạng các tác vụ như phát hiện đối tượng (detection), phân đoạn ảnh (segmentation), phân loại ảnh (classification), và ước lượng tư thế (pose estimation). Với kiến trúc mô-đun, tốc độ xử lý cao và khả năng triển khai linh hoạt, YOLOv8 là một lựa chọn phổ biến trong cả môi trường nghiên cứu và ứng dụng thực tiễn (Hình~\ref{fig:yolov8}).

\begin{enumerate}
    \item \textbf{Detection (Phát hiện đối tượng)}: Đây là tác vụ cốt lõi của YOLOv8, cho phép nhận diện và xác định vị trí các đối tượng trong ảnh thông qua các hộp bao (bounding boxes). Mô hình có thể phát hiện nhiều loại đối tượng cùng lúc, với tốc độ và độ chính xác cao, thích hợp cho các hệ thống thời gian thực như giám sát an ninh, xe tự hành, và kiểm tra chất lượng sản phẩm.

    \item \textbf{Segmentation (Phân đoạn ảnh)}: YOLOv8 hỗ trợ phân đoạn ảnh thông qua phương pháp phân đoạn thể hiện (instance segmentation), trong đó mỗi đối tượng được nhận diện không chỉ bởi hộp bao mà còn bởi vùng bao phủ chính xác theo pixel. Tính năng này rất hữu ích trong các lĩnh vực như y tế, nông nghiệp chính xác, và robot công nghiệp.

    \item \textbf{Classification (Phân loại ảnh)}: Trong tác vụ phân loại, mô hình đưa ra nhãn cho toàn bộ ảnh đầu vào, xác định xem ảnh thuộc loại đối tượng nào. YOLOv8 cho phép huấn luyện và triển khai các mô hình phân loại nhẹ, phù hợp với các bài toán như lọc ảnh, kiểm duyệt nội dung, hoặc chẩn đoán sơ bộ từ ảnh.

    \item \textbf{Pose Estimation (Ước lượng tư thế)}: YOLOv8 cung cấp khả năng ước lượng các điểm mốc (keypoints) trên cơ thể người hoặc các vật thể khác, từ đó phân tích tư thế, chuyển động hoặc hành vi. Ứng dụng phổ biến của tính năng này bao gồm theo dõi vận động viên, giám sát công nhân trong nhà máy, và hỗ trợ tương tác người–máy.
\end{enumerate}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Giao thức OPC UA}
