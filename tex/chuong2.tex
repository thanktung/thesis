...
truyền thông (ví dụ CC-Link) hoặc điều khiển chuyển động (motion) sẽ được lắp nối tiếp sang bên phải CPU. Việc lắp đặt cần đảm bảo các connector tiếp xúc chắc chắn, không để bụi bẩn hoặc rung động mạnh. 
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figures/Chapter2/plc-structure.png}
...
Để lập trình và cấu hình PLC Mitsubishi Q13UDVCPU, phần mềm được sử dụng phổ biến là GX Works2. 
Đây là môi trường phát triển tích hợp (IDE) của Mitsubishi Electric, cho phép thiết kế, viết chương trình, cấu hình phần cứng, giám sát trạng thái và chẩn đoán lỗi.
GX Works2 hỗ trợ nhiều ngôn ngữ lập trình chuẩn IEC 61131-3 như Ladder Diagram (LD), Structured Text (ST), và Function Block Diagram (FBD), giúp kỹ sư có thể lựa chọn phong cách lập trình phù hợp với ứng dụng thực tế. 

Trong quá trình cấu hình, người dùng khai báo loại CPU, module nguồn, vị trí các module I/O, module truyền thông hoặc motion tương ứng trên base unit. 
Ngoài ra, GX Works2 cũng cho phép thiết lập tham số mạng (Ethernet, CC-Link, v.v.), quản lý bộ nhớ và tải chương trình xuống CPU thông qua cổng USB hoặc Ethernet. 
Phần mềm còn cung cấp chức năng monitoring online, giúp quan sát trạng thái hoạt động của các bit, thanh ghi, timer/counter theo thời gian thực, từ đó hỗ trợ quá trình kiểm tra và gỡ lỗi nhanh chóng. 

\subsection{Giao diện người máy HMI}
Giao diện Người – Máy (Human-Machine Interface, HMI) là thiết bị cho phép người vận hành tương tác trực tiếp với hệ thống điều khiển thông qua màn hình trực quan. 
Trong hệ thống này, HMI được sử dụng để giám sát trạng thái hoạt động, hiển thị dữ liệu sản xuất, theo dõi cảnh báo và cho phép người dùng nhập các lệnh điều khiển. 
Thiết bị HMI giúp đơn giản hóa việc vận hành, giảm sai sót và tăng tính trực quan khi quản lý dây chuyền tự động hóa.  

Màn hình HMI PFXGP4501TAA thuộc dòng GP4000 Series của hãng Pro-face (Schneider Electric), là một trong những thiết bị giao diện phổ biến, được thiết kế nhỏ gọn nhưng mạnh mẽ, thích hợp kết nối với PLC Mitsubishi Q Series (Hình \ref{c2:hmi}). 

\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{figures/Chapter2/pfxgp4501taa.jpg}
...
\subsubsection{Viết chương trình cho Robot Yaskawa}
Trước khi có thể viết một chương trình cho robot, cần kiểm tra các điều kiện: nút [EMERGENCY STOP] không được nhấn và [Mode switch] ở trạng thái Teach Mode. Sau đó có thể di chuyển Robot bẳng cách nhấn Switch sau tay cầm và bật nguồn Servo, dùng các nút di chuyển trục để di chuyển.
Để tạo mới một một chương trình, ấn nút [MAIN MENU] -> JOB -> CREATE NEW JOB. Sau đó điền tên chương trình và có thể chọn Folder để lưu trữ. Kết thúc quá trình bằng nút EXECUTE.
...
Màn hình viết chương trình sẽ được hiển thị:
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/Chapter2/inform.png}
    \caption{Ngôn ngữ INFORM III}
    \label{c2:inform}
\end{figure}

\begin{itemize}
    \item \textbf{Instruction}: Hướng dẫn để thực hiện một lệnh.
    \item \textbf{Additional item}: Phần điều kiện bổ sung cho Instruction trước đó.
\end{itemize}

Các loại Instruction trong INFORM III như sau:

\begin{center}
\begin{tabular}{|l|l|}
\hline
\textbf{Type} & \textbf{Content} \\ \hline
I/O Instruction & Các lệnh điều khiển I/O \\ \hline
Control Instruction & Điều khiển luồng xử lý của chương trình \\ \hline
Operating Instruction & Điều khiển các biến trong chương trình \\ \hline
Move Instruction & Điều khiển di chuyển của tay máy \\ \hline
Shift Instruction & Di chuyển điểm dạy hiện tại \\ \hline
Adheres Instruction & Lệnh đi kèm theo lệnh khác \\ \hline
Work Instruction & Lệnh liên quan đến làm việc của robot \\ \hline
Optional Instruction & Lệnh chức năng tùy chọn \\ \hline
\end{tabular}
\end{center}

Chi tiết về các lệnh có trong \textit{[YRC1000micro OPTIONS INSTRUCTION FOR INFORM LANGUAGE RE-CKI-A468]}.


%===========================================================================================
\subsection{Hệ thống robot Hyundai}

\subsubsection{Tay máy và bộ điều khiển}

Tương tự như hệ thống của Robot Yaskawa, hệ thống Robot Hyundai cũng chia làm hai phần chính là cánh tay Robot và bộ điều khiển dùng để điều khiển Robot. Như minh họa tại Hình\ref{fig:hyun1}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/Chapter2/hyundai-diagram}
    \caption{Hệ thống Robot Hyundai cơ bản}
    \label{fig:hyun1}
\end{figure}

Hệ thống Robot Hyundai dùng tại phòng thực hành bao gồm tay máy Hyundai HH7 và bộ điều khiển Hi5a.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/Chapter2/HH7.png}
    \caption{Tay máy HH7}
    \label{fig:hyun2}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/Chapter2/hi5a.jpg}
    \caption{Bộ điều khiển Hi5a}
    \label{fig:hyun3}
\end{figure}

Teach pendant của Hi5a cũng cho phép chuyển chế độ hoạt động, điều khiển hệ thống tương tư với teach pendant của Robot Yaskawa.

\subsubsection{Viết chương trình điều khiển bằng Teach pendant}

Chọn chương trình bằng cách bấm phím [SHIFT] + [PROG] trên Teach pendant, chọn List show/hide để hiện thông tin của chương trình (Hình\ref{fig:hyun4}).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/Chapter2/hyun4.png}
    \caption{Chọn chương trình}
    \label{fig:hyun4}
\end{figure}

Một chương trình điều khiển robot thường gồm hai loại lệnh:
\begin{itemize}
    \item \textbf{Lệnh bước (step command):} dùng để di chuyển robot.
    \item \textbf{Lệnh chức năng (function command):} dùng để thực hiện thao tác.
\end{itemize}

Mỗi lệnh bao gồm phần \textit{command} và các \textit{thuộc tính (properties)}, được chia thành:
\begin{itemize}
    \item \textbf{Thuộc tính bắt buộc (base properties).}
    \item \textbf{Thuộc tính tùy chọn (optional properties).}
\end{itemize}

Khi người dùng nhập một lệnh, các giá trị mặc định sẽ được gán cho thuộc tính bắt buộc, và có thể thay đổi. Thuộc tính tùy chọn được hiển thị bằng ký tự ``\_'' trên con trỏ làm việc; khi di chuyển đến mục này, khung chính sẽ hiển thị các thuộc tính có thể chỉnh sửa.

Khi chỉnh sửa thuộc tính của lệnh, người dùng có thể:
\begin{itemize}
    \item Nhập trực tiếp bằng phím số.
    \item Chọn tham số hoặc chức năng từ menu và nhập mã (\textit{index}).
    \item Nhập theo ký tự bằng \textit{Formula} hoặc \textit{Character entry}.
\end{itemize}
...
Trong sản xuất điện tử SMT, thị giác máy đặc biệt quan trọng ở khâu kiểm tra AOI/AVI. Các mô hình học sâu hiện đại như CNN, YOLO giúp hệ thống phân loại và phát hiện linh kiện nhanh, chính xác hơn, giảm đáng kể tỷ lệ lỗi do con người.
%===========================================================================
