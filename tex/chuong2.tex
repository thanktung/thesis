...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textit{Cyclic Transmission}: truyền liên tục theo chu kỳ quét của PLC (Hình \ref{fig:cclink_cyclic}).
        \item \textit{Transient Transmission}: chỉ truyền khi có yêu cầu (Hình \ref{fig:cclink_transient}).
    \end{itemize}
...
\end{figure}
    ...
    Trạng thái hệ thống, vị trí lỗi và nguyên nhân lỗi có thể được quan sát trực tiếp trên phần mềm lập trình (Hình \ref{fig:cclink_diagnostics}).
